...
* `-i, --pick` チェックボックスを使った選択を行わず、取得したリストから表示する記事を番号で選択します。複数の番号を指定する際はスペースで区切ります。
...
* `-i, --pick` チェックボックスを使った選択を行わず、取得したリストからダウンロードする記事を番号で選択します。複数の番号を指定する際はスペースで区切ります。
